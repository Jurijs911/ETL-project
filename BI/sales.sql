-- Sales Revenue and Units Sold by Country and Quarter

SELECT LOC.country, DT.quarter, SUM(F.unit_price * F.units_sold) AS total_revenue, SUM(F.units_sold) AS total_units_sold
FROM fact_sales_order AS F
JOIN dim_location AS LOC ON F.agreed_delivery_location_id = LOC.location_id
JOIN dim_date AS DT ON F.created_date = DT.date_id
GROUP BY LOC.country, DT.quarter
ORDER BY LOC.country, DT.quarter;

--Total Sales Revenue by Currency Code

SELECT C.currency_code, SUM(F.unit_price * F.units_sold) AS total_revenue
FROM fact_sales_order AS F
JOIN dim_currency AS C ON F.currency_id = C.currency_id
