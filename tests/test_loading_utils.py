...
def test_read_inserted_dim_design_data():
    ...
    with patch('src.loading.loading_utils.create_connection',
...
def test_read_inserted_dim_currency_data():
    ...
    Parameters:
        ...
        currency_data (list): A list of lists containing currency data.
                              Each inner list should have the following format:
...
def test_insert_into_dim_staff_invalid_input():
    mock_connection = Mock()
    invalid_staff_data = [
        [1, "Invalid", "Staff1", 123,
         "Invalid Location", "email1@example.com"],
        [2, "Staff2", 456, "Invalid Department",
         "London", "email2@example.com"],
    ]
...
def test_insert_into_dim_staff_missing_columns():
    ...
    with patch('src.loading.loading_utils.create_connection',
        ...
        staff_data_with_missing_columns = [
            ...
            # Extra column
            [2, "Zenab", "Haider", "Marketing", "New York",
             "zen@example.com", "987654321", "Extra column"],
            # Too many columns
            [3, "Cam", "P", "GOD", "Chicago", "Cam@example.com",
             "123456789", "Column 1", "Column 2"],
        ]
        with pytest.raises(InputValidationError):
            insert_into_dim_staff(
...
def test_insert_into_dim_location():
    ...
    with patch('src.loading.loading_utils.create_connection',
         return_value=mock_connection):
        location_data = [
            [1, "address_line_1", "address_line_2", "district",
             "city", "postal_code", "country", "phone"],
            [2, "123 apple street", "address_line_2", "apple",
             "applecity", "ABC-123", "England", "123-456-789"],
        ]
...
def test_insert_into_dim_location_invalid_input():
    ...
    invalid_location_data = [
        [1, "Address 1", 123, "Invalid District", "Cityville",
         "12345", "Countryland", "555-123-4567"],
        [2, "123 Main St", "Apt 4", "Central District",
         "Townsville", "67890", "Countryland", 987],
    ]
...
def test_insert_into_dim_location_missing_columns():
    mock_connection = Mock()
    invalid_location_data = [
        [1, "Address 1", "District", "Cityville",
         "12345", "Countryland", "555-123-4567"],
        [2, "123 Main St", "Apt 4", "Central District",
         "Townsville", "67890", "Countryland", "555-987-6543", "Extra Column"],
    ]
...
def test_insert_into_dim_counterparty_invalid_input():
    ...
    with patch('src.loading.loading_utils.create_connection',
         return_value=mock_connection):
        counterparty_data = [
            [1, "Counterparty 1", "Address 1", "Address 2",
             "District 1", "City 1", "12345", "Country 1", 12345]
        ]
