...
#     with patch('src.loading.loading_utils.insert_into_dim_fact_sales_order') \
#         as mock_insert_into_dim_fact_sales_order:
#         mock_insert_into_dim_fact_sales_order.return_value = ['test']
#         lambda_handler(
...
def test_loading_lambda_handler_logs_no_new_data(mocker):
    ...
    with patch("src.loading.loading.read_processed_csv") as mock_read_processed_csv:
        mock_read_processed_csv.return_value = {'fact_sales_order': [], 'dim_date': [], 'dim_staff': [], 'dim_location': [], 'dim_currency': [], 'dim_design': [], 'dim_counterparty': []}
...
#     # table.txt objects
    ...
    with patch("src.loading.loading.read_processed_csv") as mock_read_processed_csv:
        ...
        lambda_handler({}, {}, test_user, test_database, test_host, test_port, test_password)

    # Check if the log_to_cloudwatch function was called with the expected arguments
    spy.assert_any_call(
        "Data has been inserted into the fact_sales_order table.",
        "/aws/lambda/loading-lambda",
        "lambda-log-stream",
